...
public class ChunkExecutorService {
    ...
    public void processChunk(List<Map<String, String>> chunk,
        ...
        // UPDATE PRODUCT
        List<Map<String, String>> updateProductRows = chunk.stream()
                .filter(r -> "updateProduct".equalsIgnoreCase(r.get("operation")))
                .collect(Collectors.toList());

        for (Map<String, String> row : updateProductRows) {
            executor.submit(() -> processUpdateProduct(row, builder, productsApi));
        }

        // DELETE PRODUCT
        List<Map<String, String>> deleteRows = chunk.stream()
                .filter(r -> "delete".equalsIgnoreCase(r.get("operation")))
                .collect(Collectors.toList());

        for (Map<String, String> row : deleteRows) {
            executor.submit(() -> processDelete(row, productsApi));
        }
    }
    ...
    private void processUpdateProduct(Map<String, String> row,
                                      ProductBuilderService builder,
                                      ProductsApi productsApi) {
        try {
            ...
            executeWithRetry(() -> {
                try {
                    ...
                    System.out.println("UPDATED PRODUCT: " + productCode);
    ...
    private void processDelete(Map<String, String> row,
                               ProductsApi productsApi) {
        try {
